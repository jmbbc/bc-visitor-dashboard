...
(async()=>{
  ...
  for(const [unit,rows] of units){const result=summarize(rows);if(!result){reasonCounts.no_valid_category=(reasonCounts.no_valid_category||0)+1;continue;}if(result.review){review.push({unit,reasons:result.reasons,sourceResponseCount:rows.length,state:result.state});for(const reason of result.reasons)reasonCounts[reason]=(reasonCounts[reason]||0)+1;continue;}const s=result.state;
  ...
  if(APPLY){for(let i=0;i<updates.length;i+=400){const batch=db.batch();for(const u of updates.slice(i,i+400))batch.set(db.doc(`overnightLocks/unit-${u.unit}`),u.data,{merge:true});await batch.commit();}for(const item of review){const ref=db.doc(`parkingReviewQueue/${item.unit}`),existing=await ref.get();if(existing.exists&&existing.data().status==='resolved')continue;const s=item.state;await ref.set({unitId:item.unit,status:'pending',reasons:item.reasons,suggestedCategory:s.category,suggestedMainUsageDays:s.dates.size,suggestedCycleStart:admin.firestore.Timestamp.fromDate(s.cycleStart),suggestedLastMainEnd:admin.firestore.Timestamp.fromDate(s.lastMainEnd),suggestedLastAnyEnd:admin.firestore.Timestamp.fromDate(s.lastAnyEnd),sourceResponseCount:item.sourceResponseCount,generatedAt:admin.firestore.FieldValue.serverTimestamp()},{merge:true});}}
  const report={generatedAt:new Date().toISOString(),apply:APPLY,responsesRead:snapshot.size,unitDocsRead:unitSnapshot.size,unitsFound:units.size,unitsReady:updates.length,unitsRequiringReview:review.length,unitsWithoutUsableState:units.size-updates.length-review.length,reasonCounts,reviewUnits:review.map(item=>item.unit)};
